...
// Runs in GitHub Actions daily at 6am ET — writes docs/intelligence.json
...
Search aggressively. Surface what matters. Be direct. Return clean plain text with absolutely no citation tags, no square bracket references, no markdown fences.`;
...
const POLY_MARKETS = [
  ...
  { label: "China Invades Taiwan 2026", slug: "will-china-invade-taiwan-in-2026",               aiKey: "taiwan_invasion", domain: "Indo-Pacific", aiProb: 8,  confidence: "MEDIUM", rationale: "Window opening but 2027 remains primary PLA readiness target. Gray zone more likely near term." },
...
async function main() {
  ...
  try {
    output.runNumber = (JSON.parse(fs.readFileSync(outPath, "utf8")).runNumber || 0) + 1;
  } catch (_) {}
...
`Search for the most significant geopolitical developments across ALL global theaters as of ${today}. Cover Middle East, Indo-Pacific, Europe/Russia/Ukraine, Americas, Africa, energy markets, technology competition, and US alliance health.

Return ONLY valid JSON. No markdown fences. No citation tags. No square bracket references. All values must be plain readable text:
{
  ...
  "unseen": "most non-obvious cross-domain pattern analysts are missing in plain text",
  "criticalWindow": "most time-sensitive decision point in next 30-90 days in plain text",
  "watchFor": ["specific trigger 1", "specific trigger 2", "specific trigger 3", "specific trigger 4"],
  "confidenceNote": "honest statement of key uncertainties in plain text",
  ...
  try {
    const data = await callClaude([{ role: "user", content:
`Search for the most significant geopolitical developments from the last 24-48 hours as of ${today}. Cast a wide global net across all regions. Surface 5-7 signals. Prioritize what is new, unexpected, or contradicts consensus.

Return ONLY valid JSON. No markdown. No citation tags. No bracket references. All text must be plain readable sentences:
{
  "signals": [
    {
      "id": "sig1",
      ...
      "signal": "non-obvious strategic implication for US interests",
  ...
  "scanNote": "one sentence on what you searched and signal quality"
    ...
    output.signals = (parsed.signals || []).map((s, i) => ({
      ...s,
      id: s.id || `sig${i+1}`,
  ...
  try {
    const data = await callClaude([{ role: "user", content:
`Based on current global conditions as of ${today}, identify the 6 most important specific observable things to watch over the next 2-4 weeks from a US strategic perspective, and the 5 most important established facts across different global domains right now.

Return ONLY valid JSON. No markdown. No citation tags. All plain text:
{
  "watchSignals": [
    "specific observable trigger to watch 1",
    "specific observable trigger to watch 2",
    "specific observable trigger to watch 3",
    "specific observable trigger to watch 4",
    "specific observable trigger to watch 5",
    "specific observable trigger to watch 6"
  ],
  "groundTruth": [
    {
      "id": "gt1",
      ...
      "summary": "2-3 sentence factual summary",
      "signal": "strategic implication for US"
    }
  ]
}`
    }], 2000);
  ...
  console.log(`Signals: ${clean.signals?.length}`);
  console.log(`Watch: ${clean.watchSignals?.length}`);
  console.log(`Ground truth: ${clean.groundTruth?.length}`);
}
